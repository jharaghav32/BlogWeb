...
const Login = () => {
  ...
  const [forms,setforms]=useState({email:'',password:''});
   ...
   const handleClicks=()=>{
    localStorage.setItem('emails',forms.email)
   }
   useEffect(()=>{
    setvalue(localStorage.getItem('email'))
    setdemos(localStorage.getItem('emails'))
   })
   console.log(value)
  return (
    <div>
  ...
  <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
    <h2 className="mb-4 text-center text-2xl font-bold text-gray-800 md:mb-8 lg:text-3xl">Login(Login using Google I have applied only google signup option for testing)</h2>

    <form className="mx-auto max-w-lg rounded-lg border">
      <div className="flex flex-col gap-4 p-4 md:p-8">
        <div>
          <label for="email" className="mb-2 inline-block text-sm text-gray-800 sm:text-base">Email</label>
          <input name="email" onChange={handleChange} value={forms.email} className="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring" />
        </div>

        <div>
          <label for="password" className="mb-2 inline-block text-sm text-gray-800 sm:text-base">Password</label>
          <input name="password" onChange={handleChange} value={forms.password} className="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring" />
        </div>
        ...
        <div className="relative flex items-center justify-center">
          ...
          <span className="relative bg-white px-4 text-sm text-gray-400"onClick={handleClicks}>Log in with social</span>
        </div>

        <button className="flex items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-8 py-3 text-center text-sm font-semibold text-gray-800 outline-none ring-gray-300 transition duration-100 hover:bg-gray-100 focus-visible:ring active:bg-gray-200 md:text-base" onClick={handleClick}>
